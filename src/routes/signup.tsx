    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-background px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="w-full max-w-md space-y-8 rounded-2xl border border-border bg-surface/50 p-8 shadow-sm sm:p-10">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Get started with MarketGen AI for free.</p>
        </div>
        ...
        <div className="mt-8 space-y-6">
          ...
          <form onSubmit={submit} className="space-y-4">
            <div>
              ...
              <input required placeholder="Full name" value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none transition-colors focus:border-primary focus:ring-1 focus:ring-primary/50" />
            </div>
            <div>
      ...
      <p className="mt-8 text-center text-sm text-muted-foreground">
        Already have an account? <Link to="/login" className="font-semibold text-foreground hover:text-primary transition-colors">Sign in</Link>
      </p>
    </div>
  );
}
